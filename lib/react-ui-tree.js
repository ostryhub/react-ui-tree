...
class UITree extends Component {
  ...
  html_onDrop = e => {
    ...
    if (!this.isDragging && this.isDropAllowed(node, e))
    {
      let transformedNode = this.transformDroppedNodeDelegate(node, e);
  ...
  isDropAllowed(node, dropEvent)
  {
    let isAllowed = typeof this.props.isDropAllowedPredicate === "function" ? this.props.isDropAllowedPredicate(node, dropEvent) : true;
    return isAllowed;
  }

  transformDroppedNodeDelegate(node, dropeEvent)
  {
    let transformer = this.props.transformDroppedNodeDelegate || function(node, dropEvent) { return node };
